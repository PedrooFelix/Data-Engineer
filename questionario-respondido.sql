...
--A tabela abaixo contém os salários mínimos
--mensais de cinco países presentes na amostra 
--de dados. Baseado nesses valores, gostaríamos
--de saber quantos usuários ganham mais de 5 
--salários mínimos em cada um desses países.

drop table amostra_salarial;
CREATE TEMPORARY TABLE amostra_salarial(id_pais,pais varchar(30),salario numeric)

insert into amostra_salarial(pais,salario)
values(12,'United States',4787.90)

insert into amostra_salarial(pais,salario)
values('India',243.52);

insert into amostra_salarial(pais,salario)
values('United Kingdom',6925.63);

insert into amostra_salarial(pais,salario)
values('Germany',6664);

insert into amostra_salarial(pais,salario)
values('Canada',5567.68);

select * from amostra_salarial

SELECT amostra_salarial.pais
	,count(*) FROM respondente
	left join pais 
	on respondente.pais_id=pais.pais_id
	inner join amostra_salarial
	on pais.nome_pais = amostra_salarial.pais
	where 
		respondente.salario>=5*amostra_salarial.salario
	group by amostra_salarial.pais
